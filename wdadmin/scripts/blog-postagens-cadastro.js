$(document).ready(function () {
    ...
    function copiarTexto() {
        var copyText = document.getElementById("inputUrl_amigavel");
    ...
    document.getElementById("copiaTexto").onclick = function () { copiarTexto() };
    ...
    monta_select("inputIdBlogCategorias", "id_blog_categorias", "descricao", "blog_categorias", "WHERE status = 1 AND descricao != 'Home'", "descricao", "", false);
    monta_select("inputIdBlogSubCategorias", "id_blog_subcategorias", "descricao", "blog_subcategorias", "WHERE status = 1 AND id_blog_categorias = " + $("#inputIdBlogCategorias").val(), "descricao", "", false, true);
    monta_select("inputIdAutores", "id_autores", "nome", "autores", "WHERE status = 1", "nome", $("#inputIdAutores").val(), "", false);
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data !== 0) {
                ...
                $("#imgImagemAtual").attr("src", vsUrl + "uploads/blog_postagem/" + data[0].imagem);
            ...
            if (data != 0) {
                ...
                for (i = 0; i < data.length; i++) {
                    $("#tabela_galeria_post tbody").append(
                        ...
                        "<td><img src='" + vsUrl + "uploads/blog_postagem_galeria/" + data[i].imagem + "' class='img-fluid' style='height:35px'></td>" +
                        "<td align=\"center\">" +
                        ...
                        "<button type=\"button\" class=\"btn btn-danger btn-sm\" onclick=\"confirma_exclusao_registro(" + data[i].id_blog_postagem_galeria + ", 'blog_postagem_galeria', 'blog_postagem_galeria', '" + data[i].imagem + "', '', '', '', '');\" data-toggle=\"tooltip\" title=\"Remover " + data[i].descricao + "\"><i class=\"far fa-trash-alt fa-fw\" aria-hidden=\"true\"></i></button>" +
                        "</td>" +
                        "</tr>"
                    );
...
function edita_blog_postagem_galeria(viIdBlogPostagemGaleria) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#imgImagemGaleriaAtual").attr("src", vsUrlSite + "uploads/blog_postagem_galeria/" + data[0].imagem);
            CloseLoading();
        },
        error: function () {
            CloseLoading();
