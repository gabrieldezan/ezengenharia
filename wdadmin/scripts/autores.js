$(document).ready(function () {
    ...
    $(this).attr("title", "WD Admin - Autores");
...
function carrega_autores() {
    ...
    $.ajax({
        url: vsUrl + "controllers/RetornaAutores.php",
        type: "POST",
        dataType: "json",
        async: false,
        success: function (data) {
            if (data !== 0) {
                $("#tabela_autores tbody").html("");
                $('#tabela_autores').DataTable().destroy();
                var table = $('#tabela_autores').DataTable({
                    ...
                    buttons: [
                        ...
                        },
                        {
                            ...
                            action: function () {
                                window.location.href = "autores/cadastro";
                            }
                        }
                    ],
                    fixedHeader: false,
                    ...
                    "columns": [
                        {"data": "id_autores"},
                        {
                            "render": function (data, type, row) {
                                return "<img src=\"" + vsUrl + "uploads/autores/" + row.imagem + "\" class=\"img-fluid\" style=\"max-height:70px;\" />";
                            }
                        },
                        {"data": "nome"},
                        {
                        ...
                        },
                        {
                            "render": function (data, type, row) {
                                return "\
                                    <a href=\"autores/cadastro/" + row.id_autores + "\" class=\"btn btn-sm btn-outline-secondary\" data-toggle=\"tooltip\" data-placement=\"left\" title=\"Editar " + row.nome + "\"><i class=\"fas fa-edit\"></i></a>\n\
                                ";
                            }
                        }
                    ],
