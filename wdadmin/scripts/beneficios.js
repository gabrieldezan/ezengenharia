$(document).ready(function () {
    ...
    $(this).attr("title", "WD Admin - Benefícios");
...
function carrega_beneficios() {
    ...
    $.ajax({
        url: vsUrl + "controllers/RetornaBeneficios.php",
        type: "POST",
        dataType: "json",
        async: false,
        success: function (data) {
            if (data !== 0) {
                $("#tabela_beneficios tbody").html("");
                $('#tabela_beneficios').DataTable().destroy();
                var table = $('#tabela_beneficios').DataTable({
                    "language": {
                    ...
                    buttons: [
                        ...
                        },
                        {
                            ...
                            action: function () {
                                window.location.href = "beneficios/cadastro";
                            }
                        }
                    ],
                    fixedHeader: false,
                    ...
                    "columns": [
                        ...
                        {
                            "render": function (data, type, row) {
                                return "<a href=\"beneficios/cadastro/" + row.id_beneficios + "\" class=\"btn btn-sm btn-outline-secondary\" data-toggle=\"tooltip\" data-placement=\"left\" title=\"Editar " + row.titulo + "\"><i class=\"fas fa-edit\"></i></a>";
                            }
                        }
                    ],
                    'columnDefs': [
