$(document).ready(function () {
    ...
    $("#form_tamanho_imagens").on('submit', (function (e) {
        ...
        /*VERIFICA O TAMANHO DA THUMB*/
        if($("#inputAlturaThumb").val() !== "" && $("#inputLarguraThumb").val() == ""){
            AvisoPersonalizado("A Largura presisa ser informada.");
            CloseLoading();
            return false;
        }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data !== 0) {
                ...
                $("#inputLarguraThumb").val(data[0].largura_thumb);
                $("#inputAlturaThumb").val(data[0].altura_thumb);
                CloseLoading();
            } else {
