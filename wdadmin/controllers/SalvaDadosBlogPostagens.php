...
$Arquivos->setNome_amigavel(str_replace('-', ' ', url_amigavel($_POST['inputTitulo'])));
$Arquivos->setPasta("blog_postagem");
...
if ($Arquivos->getErro() == 0 || $Arquivos->getErro() == 4) {
    ...
    $BlogPostagens->setStatus($_POST['inputStatus']);
    $BlogPostagens->setId_blog_categorias($_POST['inputIdBlogCategorias']);
    $BlogPostagens->setId_blog_subcategorias($_POST['inputIdBlogSubCategorias']);
    $BlogPostagens->setId_autores($_POST['inputIdAutores']);
