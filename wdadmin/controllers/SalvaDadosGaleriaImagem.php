...
$Arquivos->setPasta("galeria_imagem");
...
if ($GaleriaImagens->salva_dados()) {
    $id = $GaleriaImagens->getRetorno_dados();
    if (!empty($_POST['imagensGrupos'])) {
        $GaleriaImagens->salva_dados_grupos($_POST['imagensGrupos'], $id);
    }
    print $id;
} else {
    print 0;
