...
class GaleriaImagens extends Conexao
{
    ...
    public function edita_dados_grupos()
    {
        ...
        if ($edita_dados->rowCount() > 0) :
            $this->setRetorno_dados($edita_dados->fetchAll());
            return true;
        else :
            return false;
        endif;
    ...
    public function salva_dados_grupos($dados, $id_ig)
    {
        try {
            $pdo = parent::getDB();
            $salva_dados = $pdo->prepare('DELETE FROM imagens_grupos WHERE id_galeria_imagem = (?)');
            $salva_dados->execute(array("$id_ig"));
            foreach ($dados as $key => $dado) {
                $salva_dados = $pdo->prepare('
                    INSERT INTO imagens_grupos (
                        id_galeria_grupos,
                        id_galeria_imagem
                    ) VALUES (
                        ?,
